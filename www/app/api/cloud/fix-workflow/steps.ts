...
export async function earlyExitReportStep(
  ...
  const blob = await putWorkflowReportBlob(reportId, JSON.stringify(report, null, 2), {
...
export async function deepSecFinalizePartialStep(
  ...
  const blob = await putWorkflowReportBlob(params.reportId, JSON.stringify(report, null, 2), {
    mirrorTarget: params.progressContext?.controlPlaneMirrorTarget
  })
...
Generated by [d3k](https://d3k.dev)`
    ...
    try {
      ...
      if (report) {
        ...
        await putWorkflowReportBlob(reportId, JSON.stringify(report, null, 2), {
          mirrorTarget: progressContext?.controlPlaneMirrorTarget
        })
